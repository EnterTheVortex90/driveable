document.addEventListener('DOMContentLoaded',()=>{
  ...
  const restore=()=>{clearInterval(ticker);document.body.classList.remove('mock-active');if(!$('#questionCard'))$('#quiz').innerHTML=originalQuiz};
  const clock=()=>`${Math.floor(test.left/60)}:${String(test.left%60).padStart(2,'0')}`;
  function start(){
    document.body.classList.add('mock-active');
    const pool=[...window.Q].sort(()=>Math.random()-.5).slice(0,50).map(q=>({q,opts:q[2].map((text,i)=>({text,correct:i===q[3]})).sort(()=>Math.random()-.5)}));
